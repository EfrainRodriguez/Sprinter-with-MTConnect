...
//------------------------------------------------
//Configuracion del modulo Ethernet ENC28J60
//------------------------------------------------

//#include <EtherCard.h>

//configuracion de juan pablo
/*static byte mymac[] = {0x74, 0x69, 0x69, 0x2D, 0x30, 0x31}; // mac del modulo ethernet
static byte myip[]  = {192, 168, 10, 73};                // ip para copnectarce al modulo ethernet
static byte gwip[]  = { 192, 168, 10, 1 };               // puerta de enlace para el modulo ethernet
static byte dns[]   = { 192, 168, 10, 1};               // dns preferido
static byte mask[]  = { 255, 255, 255, 0 };             // dns preferido
*/
//mi configuracion
static byte mymac[] = {0x74, 0x69, 0x69, 0x2D, 0x30, 0x31}; // mac del modulo ethernet
static byte myip[]  = {164, 41, 45, 82};                // ip para copnectarce al modulo ethernet
static byte gwip[]  = { 164, 41, 45, 254};               // puerta de enlace para el modulo ethernet
static byte dns[]   = { 164, 41, 45, 2};               // dns preferido
static byte mask[]  = { 255, 255, 255, 0 };             // dns preferido

byte Ethernet::buffer[700];

// responder con pong
const char str_PONG_agente[] PROGMEM = "\n* PONG 500\r\n";
...
void setup()
{
  ...
  //Inicializacion del modulo Ethernet - ENC28J60
  Serial.println("\nInicializando Modulo  ENC28J60");
  if (!ether.begin(sizeof(Ethernet::buffer), mymac, 49)) {
  ...
  ether.printIp ( " IP:    " , ether.myip);   // salida de la dirección IP a Serial
  ether.printIp ( " GW:    " , ether.gwip);   // salida de la dirección de la puerta de enlace a Serial
  ether.printIp ( " DNS:   " , ether.dnsip);  // dns preferido
  ether.printIp ( " Mask:  " , ether.netmask);  // output netmask to Serial
  ether.printIp ( " DHCP:  " , ether.dhcpip); // salida de la dirección IP del servidor DHCP
  Serial.println("\n");

  SET_OUTPUT(42);// setea el pin 42 como salida para el led
   /*================================================================================================
   * ===============================================================================================
   *                              aqui están las configuraciones para el timer 5
   *                              que pretendemos usar para el adaptador MTConnect
   *                              
   *                              la función ISR associada a esta interrupcion está
   *                              arriba inmediatamente antes de la función st_init
   */
...
//------------------------------------------------
//funcion para el timer5
//------------------------------------------------
ISR(TIMER5_COMPA_vect)  //esta es la funcion de la rutina de la interrupcion. Lo que está aqui es lo que se ejecuta
{
  word pos = ether.packetLoop(ether.packetReceive());
  // check if valid tcp data is received
  if (pos) {
    char* data = (char *) Ethernet::buffer + (pos + 2);
//    Serial.print("Recibido: [");
//    Serial.print(data);
//    Serial.print("] tiempo:");
//    Serial.println(millis());
    if (strncmp("PING", data, 4) == 0) {
//      Serial.println("respondiendo PONG");
      ether.httpServerReplyAck(); // enviar ack a la solicitud
      memcpy_P(ether.tcpOffset(), str_PONG_agente, sizeof str_PONG_agente); // enviar el segundo paquete y no enviar la bandera de terminación
      ether.httpServerReply_with_flags(sizeof str_PONG_agente - 1, TCP_FLAGS_PSHACK_V);
      delay(5);
      ether.httpServerReply_with_flags(enviarDatos(msgDatos()), TCP_FLAGS_PSHACK_V);
      delay(5);
    }
  }
}
...
String msgDatos() {
  const String str_MTC_datos     =     "|build progress|0%|";
  
  str_MTC_datos       +=      "Xact|" + String(current_position[X_AXIS], 1) +
                              "|Yact|" + String(current_position[Y_AXIS], 1) +
                              "|Zact|" + String(current_position[Z_AXIS], 1) +
                              "|Extruder|" + String(current_position[E_AXIS], 1) +
                              "|extruder temp sensor|" + String(analog2temp(current_raw)) +
                              "|bed temp sensor|" + String(analog2tempBed(current_bed_raw)) +
                              "\r\n";                           
  return str_MTC_datos;
}

static word enviarDatos(String str_MTC_datos) {
  BufferFiller bfill = ether.tcpOffset();
  const char strbuf[str_MTC_datos.length()];
  str_MTC_datos.toCharArray(strbuf, str_MTC_datos.length() + 1);
